...
#%%
#folders = glob('/Users/agiovann/NEL-LAB Dropbox/NEL/Experiments/Ferret/trigger-interface/FERRETANGEL_TEST/SOUND_*')
folders = glob('/Users/agiovann/NEL-LAB Dropbox/NEL/Experiments/Ferret/Ferret_158/LEARNING_*/')        
folders.sort()
folders = folders[8:9]
        ...
        for tr, tr_whisk,  tm, ttype in zip(np.copy(traces), np.copy(traces_whisk), np.copy(times), trig_type):
            ...
            if ttype == 'CSUS' or ttype == 'CS':
                try:
                    ...
                    tr_eye_orig = tr_eye.copy()
                    tr_eye = tr_eye - np.median(tr_eye[(time_vec>-1) & (time_vec<0)])
#                    tr_eye = tr_eye - np.min(tr_eye[(time_vec>-0.4) & (time_vec<-0.05)])                    
...
#                    tr_eye = tr_eye/np.max(tr_eye[(time_vec>=0) & (time_vec<0.2)])                    
                    ...
                    if np.max(tr_whisk[(time_vec>-1) & (time_vec<0)])<1 and \
                                    (np.max(tr_eye)<3) and  ampl_USs[-1]>0.15: # recently added to patch the issue of outlier US amplitude
                                    # and (np.std(tr_eye_orig[(time_vec>-1) & (time_vec<0)]) < 0.01):
...
pl.xlim([-.3,0.9])
pl.ylim([-.1,1.2])
pl.ylim([-0.75,1.75]) #covers a wider range of y values
